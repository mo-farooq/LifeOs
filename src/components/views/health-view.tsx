...
import { CharacterStats, Skill, Supplement, WaterConfig, NutritionConfig, BlocksConfig } from "@/types";
...
export default function HealthView({
  ...
  const showSupplement = blocksConfig?.supplementStack ?? true;
  const showWater = blocksConfig?.waterCoach ?? true;
  const showNutrition = blocksConfig?.macroMonitor ?? true;

  const leftSpan = !showNutrition ? "lg:col-span-12" : "lg:col-span-6";
  const rightSpan = (!showSupplement && !showWater) ? "lg:col-span-12" : "lg:col-span-6";
              ...
              <div className="space-y-1.5 max-h-[220px] overflow-y-auto pr-1">
                {supplements.map((item) => {
                  ...
                  return (
                    <div 
                      key={item.id} 
                      className={`group flex items-center justify-between px-4 py-3 border rounded text-sm font-mono transition-all duration-150 ${
                        isTaken 
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3.5">
                        <button
                          ...
                          className={`w-5.5 h-5.5 border flex items-center justify-center rounded-sm transition-all ${
                            isTaken 
                              ? "bg-zinc-50 border-zinc-100 text-zinc-950" 
                              : "border-zinc-800 bg-transparent hover:border-zinc-650"
                          }`}
                        >
                          {isTaken && <Check className="h-3.5 w-3.5 stroke-[3.5] text-zinc-950" />}
                        </button>
                        <div className="space-y-0.5">
                          <p className={`text-sm font-semibold ${isTaken ? "line-through text-zinc-500" : "text-zinc-200"}`}>
                            {item.name} <span className="text-xs text-zinc-500">({item.dose})</span>
                          </p>
                          <div className="flex items-center gap-2.5 text-[10px] font-mono text-zinc-500 uppercase tracking-wide">
                        ...
                        <button
                          ...
                          className={`text-xs font-mono px-2.5 py-0.5 rounded border transition-colors ${
                            item.low 
                              ...
                              : "border-zinc-900 text-zinc-555 hover:text-zinc-400"
                          }`}
                        >
                          {item.low ? "LOW" : "OK"}
                        </button>
                ...
                {supplements.length === 0 && (
                  <div className="text-center py-4 text-xs font-mono uppercase tracking-widest text-zinc-650">No supplements in dispatcher database.</div>
                )}
              </div>
          ...
          {showWater && (
            <Card className="bg-[#0a0a0a] border-zinc-800 rounded-md">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1.5 bg-zinc-100 hover:bg-white text-zinc-950 rounded text-xs font-mono font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all active:scale-95"
                >
                  <Plus className="h-3.5 w-3.5 stroke-[2.5]" /> 250ML
                </button>
                <button
                  onClick={() => handleLogWater(-250)}
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div className="space-y-3.5 border-t border-zinc-900 pt-3">
                <span className="text-xs font-mono uppercase tracking-widest font-bold text-zinc-550 flex items-center gap-1.5">
                  <Sliders className="h-4 w-4" /> COACH PARAMETERS
                </span>
                ...
                <div className="text-xs font-mono text-zinc-555 uppercase tracking-wide leading-relaxed border-t border-zinc-900/50 pt-2.5">
        ...
        {showNutrition && (
          ...
          <Card className="bg-[#0a0a0a] border-zinc-800 rounded-md">
            ...
            <CardContent className="p-6 space-y-6">
              {editingNutritionTargets ? (
                ...
                <>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {/* Protein */}
                    <div className="border border-zinc-900 bg-[#000000]/40 p-3.5 rounded space-y-1.5">
                      <div className="flex justify-between text-xs font-mono text-zinc-555 uppercase tracking-wider font-bold">
                        <span>PRO</span>
                      ...
                      <div className="text-right text-xs font-mono text-zinc-555 font-bold">{protPercent}%</div>
                    </div>
                    ...
                    <div className="border border-zinc-900 bg-[#000000]/40 p-3.5 rounded space-y-1.5">
                      ...
                      <div className="text-right text-xs font-mono text-zinc-555 font-bold">{carbPercent}%</div>
                    </div>
                    ...
                    <div className="border border-zinc-900 bg-[#000000]/40 p-3.5 rounded space-y-1.5">
                      ...
                      <div className="text-right text-xs font-mono text-zinc-555 font-bold">{fatPercent}%</div>
                    </div>
                  </div>
                </>
              )}
              ...
              {/* Log panel */}
              <div className="space-y-3.5 border-t border-zinc-900 pt-3">
                <span className="text-xs font-mono uppercase tracking-widest font-bold text-zinc-500 flex items-center gap-1.5">
                  <PlusCircle className="h-4 w-4" /> LOG INTENDED NUTRITION
                </span>
                ...
                <div className="flex flex-wrap gap-2 pt-2 border-t border-zinc-900/50">
                  <button
                    ...
                    className="px-2.5 py-1 border border-zinc-905 bg-[#000000]/60 hover:bg-[#0a0a0a] text-xs font-mono text-zinc-400 rounded uppercase font-bold transition-all active:scale-95"
                  >
                  ...
                  <button
                    ...
                    className="ml-auto px-2.5 py-1 border border-zinc-905 bg-red-950/10 hover:bg-red-950/20 text-xs font-mono text-red-400 rounded uppercase font-bold transition-all active:scale-95"
                  >
                    RESET
                  </button>
                </div>
